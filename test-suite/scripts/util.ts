...
export const connection = await NearConncetion.build(config)
export var master_acc: Account

try{
  master_acc = await getAccFromFile(config.master!.keyPath, connection)
}catch(e){
  Logger.error("error geting master account from the file")
...
export async function deployKatherineFundraising(
  ...
  katherine_fee_percent: number
    ...
    try{
      ...
      let katerine_ctr = new Contract(master_acc, contract_spec.katerine_ctr, "katerine_ctr")
      let contract_account = await createSubAccount(master_acc, connection, "katerine-ctr")
      let owner_account = await createSubAccount(master_acc, connection, "katerine-owner")
      ...
      await deployContract(contract_account, contract_spec.katerine_ctr)
      ...
      Logger.error("error deploying katerine contract")
...
export async function deployTestMetapool(
  ...
  try{
    let contract_spec = await import("../contracts/contract-spec.js") 
    let metapool_ctr = new Contract(master_acc, contract_spec.metapool_ctr, "metapool_ctr")
    let contract_account = await createSubAccount(master_acc, connection, "metapool-ctr")
    let owner_account = await createSubAccount(master_acc, connection, "metapool-owner")
    ...
    Logger.error("error deploying metapool contract")
    Logger.error(e as any, true)
  }
}
...
await import(join(__dirname, executing_script + ".ts"))