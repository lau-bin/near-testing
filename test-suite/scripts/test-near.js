...
(async function main() {
  ...
  //Begin test
  try {
    console.log("Begin test: ddos attack");
    ...
    await testCtr["new"](
      {
        account_id: testAcc.accountId //not used in test
      }
    )

    // instantiate test contract 2
    console.log("instantiate test contract copy for user2")
    await test2Ctr["new"](
      {
        account_id: testAcc2.accountId //not used in test
      }
    )
    ...
    // transfer 50 near to testAcc, mode ddos
    console.log("transfer 50 near to testAcc, mode ddos")
    metaPoolCtrUser["ft_transfer_call"](
      {
    ...
    console.log(nearAPI.utils.format.formatNearAmount(user1Balance_1))
    console.log("User1 balance after transfer:")
    console.log(nearAPI.utils.format.formatNearAmount(user1Balance_2))
    console.log("testAcc balance before transfer:")
    console.log(nearAPI.utils.format.formatNearAmount(testAccBalance_1))
    console.log("testAcc balance after transfer:")
    console.log(nearAPI.utils.format.formatNearAmount(testAccBalance_2))

    assert(user1Balance_1 === nearAPI.utils.format.parseNearAmount("100"), "wrong initial deposit for user1")
    assert(user1Balance_2 === nearAPI.utils.format.parseNearAmount("100"), "user1 balance should have been refunded")
    assert(testAccBalance_1 === "0", "testAcc balance should be 0 before transfer")
    ...
    // try to transfer 10 near to testAcc2
    console.log("try to transfer 10 near to testAcc2")
    await metaPoolCtrUser["ft_transfer_call"](
      {
        receiver_id: testAcc2.accountId,
        amount: nearAPI.utils.format.parseNearAmount("10"),
        memo: undefined,
        msg: ""
      },
      undefined,
    ...
    // check user1 balance
    console.log("check user1 balance")
    let user1Balance_3 = await metaPoolCtrUser["ft_balance_of"](
      {
        account_id: user1Acc.accountId
      }
    )

    console.log("user1 balance after second transfer:")
    console.log(nearAPI.utils.format.formatNearAmount(user1Balance_3))
    console.log("testAcc2 balance before second transfer:")
    console.log(nearAPI.utils.format.formatNearAmount(testAcc2Balance_1))
    console.log("testAcc2 balance after second transfer:")
    console.log(nearAPI.utils.format.formatNearAmount(testAcc2Balance_2))

    assert(user1Balance_3 === nearAPI.utils.format.parseNearAmount("90"), "10 near transfer should have been successfull")
    assert(testAcc2Balance_1 === "0", "testAcc2 should start with 0 near")
    assert(testAcc2Balance_2 === nearAPI.utils.format.parseNearAmount("10"), "testAcc2 should have been transfered 10 near")
